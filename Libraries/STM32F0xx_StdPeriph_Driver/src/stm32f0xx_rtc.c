...
ErrorStatus RTC_SetTime(uint32_t RTC_Format, RTC_TimeTypeDef* RTC_TimeStruct)
{
	...
	{
		tmpreg = (uint32_t)(((uint32_t)RTC_ByteToBcd2(RTC_TimeStruct->RTC_Hours)
				<< 16)
...
void RTC_GetTime(uint32_t RTC_Format, RTC_TimeTypeDef* RTC_TimeStruct)
{
	...
	RTC_TimeStruct->RTC_Hours = (uint8_t)((tmpreg & (RTC_TR_HT | RTC_TR_HU))
			>> 16);
	RTC_TimeStruct->RTC_Minutes = (uint8_t)((tmpreg & (RTC_TR_MNT | RTC_TR_MNU))
			>> 8);
		...
		RTC_AlarmTypeDef* RTC_AlarmStruct)
{
	...
	RTC_AlarmStruct->RTC_AlarmTime.RTC_Seconds = (uint32_t)(tmpreg
			& (RTC_ALRMAR_ST |
			RTC_ALRMAR_SU));
	RTC_AlarmStruct->RTC_AlarmTime.RTC_H12 = (uint32_t)((tmpreg & RTC_ALRMAR_PM)
			>> 16);
		...
		uint32_t RTC_SmouthCalibMinusPulsesValue)
{
	...
	if((RTC->ISR & RTC_ISR_RECALPF) == RESET)
	{
		...
		RTC->CAL = (uint32_t)((uint32_t)RTC_SmoothCalibPeriod
				| (uint32_t)RTC_SmoothCalibPlusPulses
				| (uint32_t)RTC_SmouthCalibMinusPulsesValue);
